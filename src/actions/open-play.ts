...
export async function createOpenPlaySignupAction(
  ...
  if (!user) {
    const next = `/open-play?session=${encodeURIComponent(sessionId)}`;
    redirect(`/sign-in?next=${encodeURIComponent(next)}`);
  }

  if (!UUID_PATTERN.test(sessionId)) {
    return failure("That open-play session is invalid.");
  }
  ...
  const { data, error } = await supabase.rpc("create_open_play_signup", {
    p_session_id: sessionId,
  });

  if (error || !data || typeof data !== "object") {
    return failure(
      "This open-play session is no longer accepting registrations. Refresh the schedule and try again.",
    );
  }
  ...
  redirect(`/open-play/payment/${encodeURIComponent(reference)}`);
}

export async function submitOpenPlayPaymentProofAction(
  _previousState: BookingActionState,
  formData: FormData,
): Promise<BookingActionState> {
  const reference = String(formData.get("openPlayReference") ?? "").trim();
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  if (!user) {
    redirect(
      `/sign-in?next=${encodeURIComponent(`/open-play/payment/${reference}`)}`,
    );
  }
  ...
  if (!/^TPC-\d{4}-\d{4,}$/.test(reference)) {
    return failure("That open-play reference is invalid.");
  }
  ...
  const receiptPath = `${user.id}/open-play/${signup.id}/${randomUUID()}.${extension}`;
